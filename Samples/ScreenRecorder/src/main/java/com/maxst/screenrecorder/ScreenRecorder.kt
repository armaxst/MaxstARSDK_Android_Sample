...
class ScreenRecorder constructor(context: Context, mediaProjectionManager: MediaProjectionManager, data: Intent) : WeakRefHandler.MessageListener {
	...
	companion object {
		...
		private const val MSG_WRITE_AUDIO = 2
	...
	var videoConfig = VideoConfig.getDefaultConfig(context as Activity)
	var audioConfig = AudioConfig.getDefaultConfig()
...
//		}
	...
	private fun queuePCMToInputBuffer(codec: MediaCodec?, index: Int) {
		...
		while (pendingAudioInputBuffer.isNotEmpty()) {
			val inputIndexWithbuffer: Pair<Int, MediaCodec?> = pendingAudioInputBuffer.pollLast()
